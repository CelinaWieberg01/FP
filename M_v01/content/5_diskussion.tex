...
ist damit zu erklären, dass die Signale wie in \autoref{fig:delayskizze} dargestellt eine Breite besitzen und der Überlapp, also die Menge der gleichzeitig eintreffenden Counts, abnimmt. Der eingestellte Delay von \qty{0}{\nano\second} ist somit begründet.
...
Da die Zeit der Myonen zwischen Eintritt und Zerfall in diesen Kanälen sehr kurz ist, könnte es vorstellbar sein, dass die geringe Anzahl der Counts auf elektrotechnische Effekte zurückzuführen ist, die durch eine nicht perfekte Justierung des Delays bedingt sind.

Die gefundene Lebensdauer $\tau$ beträgt
\begin{equation*}
    \tau = \qty{1.739(105)}{\micro\second}
\end{equation*}

und hat somit eine Abweichung vom Literaturwert $\tau_\text{lit} = \qty{2.196}{\micro\second}$ von etwa \qty{20.8}{\percent}.
Es sei hier angemerkt, dass die Lebensdauer unter Berücksichtigung aller Messwerte bei $\qty{2.345(147)}{\micro\second}$ liegen würde, was eine Abweichung von nur \qty{6.7}{\percent} zum Literaturwert bedeuten würde.
Allerdings ist diese Auswertung durch die nicht erklärbaren Messwerte bei geringen Zeiten verfälscht.
