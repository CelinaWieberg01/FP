...
\end{equation*}
dabei ist $A$ die Amplification \cite{eqns}. In einem idealen OP-Amp ist dieser unendlich hoch, was jede noch so kleine Signaldifferenz verstärken würde. In der Realität ist dies nicht möglich, 
...
Ein Inverting-Amplifier invert eingehende Signale. Über die Kirchhoff'schen Regeln \cite{kirchhoff} kann nachgerechnet werden, dass 
\begin{equation}
    A = - \frac{R_2}{R_1}
    \label{eq:inverter}
\end{equation}
...
Diese Schaltung kann ein eingehendes Signal integrieren, was anhand der Knotenregeln \cite{kirchhoff} bewiesen werden kann, sodass gilt
...
Ein Schmitt-Trigger ist ein sogenannter Komparator, der ein binäres Signal in Höhe der beiden Betriebsspannungen $U_\text{S}$ gibt, sobald das Eingangssignal klein oder groß genug ist.
