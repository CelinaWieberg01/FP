...
Teilchen durch die Bethe‑Bloch‑Gleichung \cite{bethe} genähert,
...
wobei \(z\) die Ladungszahl des Projektils, \(N\) die Atomdichte des Targets, \(Z\) die Kernladungszahl des Targetmaterials, \(m_0\) 
...
bestimmt werden kann. Die Atomdichte \(N\) wird durch die Materialdichte \(\rho\), die molare Masse \(M\) und die Avogadrozahl \(N_A\) über
...
Unter der Annahme der Einzelstreuung und der Vernachlässigung quantenmechanischer Feinheiten ergibt sich für den Rutherford‑Wirkungsquerschnitt \cite{rutherford} 
die Form

\begin{equation}
\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega}(\theta)
= \left(\frac{z Z e^2}{4\pi\varepsilon_0 E_\alpha}\right)^{\!2}\frac{1}{\sin^4(\theta/2)},
\label{eq:rutherford}
\end{equation}
...
\begin{equation}
I(\theta) \;=\; I_0\, N d\, \frac{\mathrm{d}\sigma}{\mathrm{d}\Omega}(\theta)\,\Delta\Omega\,\varepsilon 
\label{eq:I0}
\end{equation}\cite{lumin}

verknüpft ist. Hierbei bezeichnet \(I_0\) den einfallenden Teilchenstrom, \(x\) die Folienstärke, 
...
\begin{equation}
\Delta\Omega = 4 \arctan\left(\frac{b_x}{2l}\right)\arctan\left(\frac{b_x}{2l}\right)
\label{eq:omega}
\end{equation}
...
des Kerns führt zu Abweichungen bei großen Streuwinkeln und ermöglicht Rückschlüsse auf den effektiven Kernradius \cite{kernradien}.
...
Für die charakteristische RMS‑Ablenkung \(\theta_0\) wird häufig die Highland‑Formel \cite{highland} verwendet,
